...
The bundle registers `.approj` as an Apollo Project document using the exported `com.mat1jaczyyy.apollostudio.project` type. Finder can open a project in a new or running app. Avalonia file-activation events are subscribed before startup and queued until the first window is ready. Reopening the current file preserves unsaved edits; opening another asks to save/discard/cancel before replacing the project and its editor windows. Requests wait for a pending startup crash-recovery choice, existing message or native file picker, and Cancel clears the queued batch. Unreadable files preserve the current workspace. The updater helper does not register document types.

The bundle's declared minimum is macOS 12, matching the .NET native deployment target; current upstream support starts at macOS 14. Compatibility on 12/13 is unverified.
...
**Known launch limitation on the tested macOS 15.2 machine:** fresh composed Unicode (`ž`, NFC) installation paths exposed a Launch Services resolution failure, also reproduced by a minimal native app without .NET/Avalonia. Decomposed `z` + caron (NFD) installations passed Finder launch and actual signed updater relaunch with exact process/UI verification. Renaming a previously launched installation back to NFC could work, while a fresh NFC copy with the product ID launched another registered validation copy instead. Registration history matters; arbitrary Unicode installation paths remain unqualified. ASCII paths are the simplest tested choice. Unicode project file paths work independently of this prelaunch limitation. See [normalization investigation and reproducer](../Tests/Packaging/UNICODE-LAUNCH.md).

The native local updater test at an ASCII installation path containing spaces verified signed whole-bundle replacement, retention of the previous app, project/custom-connector preservation, staging cleanup and a responsive replacement UI at the exact expected process path. A successful relaunch command alone does not prove the new UI is running at the intended path; see the report's separate qualification of the Unicode-path test. A follow-up signed-bundle matrix passed 23 failure/recovery checks, including read-only installation and real mounted-DMG rejection, signature tampering, rename failure and rollback after an injected launch failure.

Finder `.approj` double-click passed with the app closed and running, including a Unicode filename, focused BPM editing, Cancel and Save-before-open. The exact cold-launched app path and a new process ID were verified before UI inspection. The separate NFC **installation-directory** failure reproduced with a fresh bundle identity; changing only its parent directory entry to NFD allowed the unchanged signed app to launch.

Portable checks additionally cover metadata, archive validation, architecture/release selection, legacy connectors and rollback. The CI workflow includes headless editor/Quit/activation checks and production-entry-point verification alongside publishing and virtual MIDI. The updated workflow has not been run remotely. Browser-download quarantine, installation/Gatekeeper approval, Dock launch, physical MIDI/Ableton, interruption/late-crash recovery and OS-floor checks remain pending. No artifacts were released in this local validation pass.
