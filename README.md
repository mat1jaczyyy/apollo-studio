...
macOS has separate Intel (`osx-x64`) and Apple Silicon (`osx-arm64`) targets for both Apollo and its updater. On a Mac, run `sh Publish/publish.sh osx-x64`, `sh Publish/publish.sh osx-arm64`, or `sh Publish/publish.sh all` from the repository root. Omitting the argument retains the Intel default. Packaging requires the pinned .NET SDK, Xcode Command Line Tools and Python 3. It uses free ad-hoc signing, with no Apple account or certificate required.

For normal installation, use `Dist/Apollo-Mac.dmg` (Intel) or `Dist/Apollo-Mac-arm64.dmg` (Apple Silicon), then drag `Apollo Studio.app` into Applications. The app launches from Finder or the Dock without Terminal. Corresponding `*-app.zip` archives serve app-bundle updates. The legacy `Apollo-Mac.zip` / `Apollo-Mac-arm64.zip` archives retain the loose-executable layout for existing installations; each updater selects its own format and processor architecture. Optional legacy `.pkg` installers still require `fileicon` and WhiteBox Packages: set `MACOS_LEGACY_PKG=1` when running the publishing script.

Downloaded ad-hoc builds are not notarized and need macOS's per-app **Privacy & Security → Open Anyway** approval. See [macOS publishing and signing](Publish/MACOS.md) for the zero-cost distribution choices, migration details and pending Mac tests.
