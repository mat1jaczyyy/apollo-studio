...
Finder `.approj` double-click passed with the app closed and running, including a Unicode filename, focused BPM editing, Cancel and Save-before-open. The exact cold-launched app path and a new process ID were verified before UI inspection. The separate Unicode **installation-directory** failure reproduced again with a fresh bundle identity.
